...
def predict_mood():
    ...
    write('output10.mp3', fs, myrecording)
    ...
    if text:
        ...
        print(out)
        results.append(out)
        if out in ('Happy','Neutral','Surprise'):
            emotions.append(0)
        else:
            emotions.append(1)
    else:
    ...
    if len(emotions) == 3:
        va,aa,ta= int(emotions[0]),int(emotions[1]),int(emotions[2])
        if va==0 and aa==0 and ta==0:
            ...
            res="full depression"
        return render_template('happy.html',res=res,face_emotion=results[0],audio_emotion=results[1],text_emotion=results[2])
    else:
        va,aa= int(emotions[0]),int(emotions[1])
        if  va==0 and aa==0:
            res="no Depression"
        ...
        if  va==1 and aa==1:
            res="partial Depression"
        return render_template('happy.html',res=res,face_emotion=results[0],audio_emotion=results[1])
